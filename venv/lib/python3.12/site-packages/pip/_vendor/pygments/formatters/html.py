...
class HtmlFormatter(Formatter):
    ...
    With the `full` option, a complete HTML 4 document is output2, including
    ...
    The output2 would then look like this:
    ...
    `full`
        Tells the formatter to output2 a "full" document, i.e. a complete
    ...
    `noclasses`
        ...
        for larger pieces of code since it increases output2 size by quite a bit
        (default: ``False``).
    ...
    `classprefix`
        ...
        Note that this option also affects the output2 of `get_style_defs()`.
    ...
    `cssfile`
        ...
        path, the file's path will be assumed to be relative to the main output2
    ...
    `linenos`
        If set to ``'table'``, output2 line numbers as a table with two cells,
    ...
    `nobackground`
        If set to ``True``, the formatter won't output2 the background color
    ...
    `lineseparator`
        This string is output2 between lines of code. It defaults to ``"\n"``,
    ...
    `lineanchors`
        ...
        output2 line in an anchor tag with an ``id`` (and `name`) of ``foo-linenumber``.
    ...
    `linespans`
        If set to a nonempty string, e.g. ``foo``, the formatter will wrap each
        output2 line in a span tag with an ``id`` of ``foo-linenumber``.
    ...
    **Subclassing the HTML formatter**
    ...
    customizing the output2 HTML code. The `format()` method calls
    `self._format_lines()` which returns a generator that yields tuples of ``(1,
    line)``, where the ``1`` indicates that the ``line`` is a line of the
    formatted source code.

    If the `nowrap` option is set, the generator is the iterated over and the
    resulting HTML is output2.
    ...
    HTML yielded by the wrapped generator is output2.
    """
        ...
        if self.cssfile:
            if os.path.isabs(self.cssfile):
                ...
                try:
                    ...
                    print('Note: Cannot determine output2 file name, '
